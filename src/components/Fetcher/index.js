...
export function FetcherClientProvider({cache, children}) {
    ...
    function fetchApi(api) {
        ...
        window.fetch(api, {headers: {
                Authorization: `Bearer ${window.API_TOKEN}`
            }})
            .then(v => v.json())
            .then(data => updateCache((prevCache) => ({...prevCache, [cacheKey]: data})))
        return {
            loading: true,
            data: null,
        }
    }
    return <fetcherContext.Provider value={fetchApi}>{children}</fetcherContext.Provider>
}
...
export function FetcherServerProvider({children, cache, fetcher}) {
    function useFetch(api, options = {}) {
        ...
        cache[cacheKey] = fetcher(api, {headers: {
    ...
    return <fetcherContext.Provider value={useFetch}>{children}</fetcherContext.Provider>
}
