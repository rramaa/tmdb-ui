        ...
        let intervalId = setInterval(() => {
            ...
            updateTranslate((prevState) => {
                if(prevState === finalTranslate) {
                    return 0
                } else if(items.length - (prevState / IMAGE_WIDTH) === fullImagesAccommodated + 1) {
...
function CarouselItem({title, image, id, baseRoute}) {
    return <Link to={`${baseRoute}/${id}`}>
        <div className={"carousel-item"}>
            <Image className={"c-img"} imagePath={image} alt={title} size={"w500"}/>
