...
export default function Wishlist() {
    ...
    return (
        <>
            {wishlist.movie.length > 0 ? <WishlistGroup items={wishlist.movie} type={"movie"} title={"Movies"}/> : "No movies add to wishlist yet"}
            <hr />
            {wishlist.tv.length > 0 ? <WishlistGroup items={wishlist.tv} type={"tv"} title={"TV Series"}/> : "No tv series add to wishlist yet"}
            <hr />
            {wishlist.person.length > 0 ? <WishlistGroup items={wishlist.person} type={"person"} title={"Persons"}/> : "No persons add to wishlist yet"}
        </>
    )
}
    ...
    return (
        <>
            ...
            <div className={"wl-group"}>
                {items.map(id => <WishlistItem id={id} type={type} key={id}/>)}
            </div>
        </>
    )
}
